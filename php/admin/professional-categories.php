<?php
$activeNav = 'professional-categories';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_id'])) {
        try {
            db()->prepare('DELETE FROM professional_categories WHERE id=?')->execute([(int) $_POST['delete_id']]);
        ...
        if ($name !== '') {
            $slug = slugify($name);
            db()->prepare('INSERT IGNORE INTO professional_categories (name, slug, description, display_order) VALUES (?, ?, ?, ?)')
                ->execute([$name, $slug, trim($_POST['description'] ?? '') ?: null, (int) ($_POST['display_order'] ?? 0)]);
            flash('success', 'Category added.');
        }
    }
    redirect('/admin/professional-categories.php');
}

$categories = db()->query('SELECT * FROM professional_categories ORDER BY display_order ASC')->fetchAll();
?>
<h1 class="display">Professional Categories</h1>
